...
Usage:
    ...
    python screener.py --top 50               # Show top 50 by volume
...
def show_top_movers(markets: List[Dict], count: int = 15, target: Console = None) -> None:
    ...
    # Exclude settled (YES <5% or >95%) and low-volume (<$250K) markets
    movers = [
        m for m in markets
        if abs(m["one_day_chg"]) > 0.001
        and 0.05 <= m["yes"] <= 0.95
        and m["volume"] >= 250_000
    ]
...
def export_to_image(
    ...
    timestamp: str,
    top_n: int = 30,
    output_path: str = "screener.png",
) -> str:
    """Render the screener tables as a styled PNG image.
    ...
    # Prepare Top Markets by 7D Volume data
    vol_sorted = sorted(markets, key=lambda m: m["volume_1w"], reverse=True)[:top_n]
    ...
    # Prepare Top Movers data (exclude settled and low-volume markets)
    movers = [
        m for m in markets
        if abs(m["one_day_chg"]) > 0.001
        and 0.05 <= m["yes"] <= 0.95
    ...
    # Calculate image dimensions — use generous height, crop later
    table_width = sum(vol_widths) + 20
    img_width = table_width + padding * 2
    # Estimate max height (3 lines per row worst case)
    max_rows = len(vol_rows) + len(mover_rows) + 10
    img_height_est = padding * 2 + 120 + max_rows * row_height * 3
    ...
    try:
        ...
        summary_y = padding + 35
        stats_text = (
            f"Markets: {len(markets)}    "
            f"Total Volume: {fmt_vol(total_vol)}    "
            f"7D Volume: {fmt_vol(total_vol_1w)}    "
            f"Avg Spread: {avg_spread*100:.2f}%"
        )
        draw.text((padding, summary_y), stats_text, fill=C["text"], font=font)

        # Divider
        div_y = summary_y + 30
        draw.line(
            [(padding, div_y), (img_width - padding, div_y)],
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        while True:
            ...
            if save_img:
                ...
                filename = f"screener{tag_part}.png"
                path = export_to_image(
                    filtered, tag_slug, timestamp,
                    top_n=top_n, output_path=filename,
                )
                if path:
                    console.print(f"[bold green]Saved screener image → {path}[/bold green]")
                    console.print(f"[dim]{len(filtered)} markets, {timestamp}[/dim]")
                return

            # Terminal output
            show_summary(filtered, tag_slug, timestamp)
            console.print()
            show_top_volume(filtered, count=top_n)
            console.print()
...
def main():
    ...
    while i < len(args):
        if args[i] in ("--interval", "-i") and i + 1 < len(args):
            ...
            i += 2
        elif args[i] in ("--top",) and i + 1 < len(args):
            top_n = int(args[i + 1])
            i += 2
        elif args[i] in ("--min-vol",) and i + 1 < len(args):
            min_vol = float(args[i + 1])
            i += 2
        elif args[i] in ("--refresh", "-r", "--img"):
            i += 1
        else:
